...
"""
Evaluate selective net on a batch of MIP instances:
------------------------------------------------
...
from divinng_selectivenet_gpu import DivingGCN_selective, get_device,reconstruct_integer_variables
import config
...
def evaluate(model_path: str, data_dir: str):
    ...
    # 2. 载入模型
    ckpt = torch.load(model_path, map_location=device)# Assuming n_bits from DivingGCN init
    model = DivingGCN_selective(input_dim=config.TRAIN_INPUT_DIM, hidden_dim=config.TRAIN_HIDDEN_DIM, output_dim=config.TRAIN_OUTPUT_DIM,n_bits=config.TRAIN_N_BITS).to(device)
    ...
    for inst in tqdm(dataset, desc="Evaluating"):
        ...
        # --- 推断 bit logits → 变量取值 ---
        with torch.no_grad():
            logits,selection_scores = model(x, ei, n_vars, ea)
        ...
        # --- 混合预测值: selection_score > 0.5用模型预测，否则用SCIP解 ---
        pred_vals = reconstruct_integer_variables(logits, vinfo)  # 模型原始预测
        selection_mask = (selection_scores > 0.5).float()         # 选择掩码 [n_vars]
        
        # 混合取值 = 模型预测 * 掩码 + SCIP解 * (1 - 掩码)
        mixed_vals = pred_vals * selection_mask + scip_sol * (1 - selection_mask)

        # --- 目标函数使用混合值计算 ---
        coeffs = [v["obj"] for v in vinfo]
        model_obj = compute_linear_obj(coeffs, mixed_vals)  # 修改此处传入mixed_vals
        ...
        if int(mask_disc.sum()) > 0:
            ...
            hit = (mixed_vals[mask_disc] == scip_sol[mask_disc]).float()  # 使用mixed_vals
        ...
        if int(mask_cont.sum()) > 0:
            rel_err = torch.abs(mixed_vals[mask_cont]-scip_sol[mask_cont]) / (torch.abs(scip_sol[mask_cont]) + 1e-6)
